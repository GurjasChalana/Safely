...
// No banner argument needed — the conv area has a unique
// DOM id so we can find and remove it without a reference.
export function closeConversation(): void {
  stopListening();
  document.getElementById(AREA_ID)?.remove();
  _active  = false;
  history  = [];
  // Tell the background to stop any audio it queued for this conversation.
  chrome.runtime.sendMessage({ type: 'STOP_CONV_AUDIO' } satisfies SafelyMessage).catch(() => {});
}
